...
if (isset($_GET['typeid'])) {
  ...
  $name = '';
  if ($typeid) {
    $ptrow = sqlQuery("SELECT code FROM codes WHERE " .
      "id = '$typeid'");
    $name = addslashes($ptrow['code']);
  }
?>
...
function selcode(typeid) {
 location.href = 'services.php<?php
...
<form method='post' name='theform' action='services.php<?php
echo "?order=$order&labid=$labid";
if (isset($_GET['formid' ])) echo '&formid='  . $_GET['formid'];
if (isset($_GET['formseq'])) echo '&formseq=' . $_GET['formseq'];
?>'>

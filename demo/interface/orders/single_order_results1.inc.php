...
function generate_order_report($orderid, $input_form=false, $genstyles=true) {
  ...
  if(isset($_POST['form_save']))
  {
	 ...
	 foreach($_POST['id'] as $selected){
		...
		$testValue= $_POST['val'][$j];
		$abn= $_POST['abn'][$j];
		$sample= $_POST['sample'][$j];
		
		 sqlStatement("update procedure_result set result='$testValue',abnormal='$abn',sample='$sample' where procedure_result_id='$selected'");
...
<table width='100%' cellpadding='2' cellspacing='0'>
 ...
 <tr class='head'>
  ...
  <td colspan='9'><?php echo xlt('Results'); ?></td>
 </tr>
...
<?php 
  ...
  while ($row = sqlFetchArray($res)) {
    ...
    $query = "SELECT " .
      "ps.result_code, ps.result_text, ps.abnormal, ps.result, ps.range,ps.procedure_result_id, " .
    ...
    foreach ($rrows as $rrow) {
	  ...
	  $result_id    = empty($rrow['procedure_result_id'          ]) ? '' : $rrow['procedure_result_id'];
	 ...
	 //if($result_lab=='yes') {$bgcolor="pink";}
 //else{	  
      $bgcolor = "#ddddff";
//}
...
if($result_lab=='yes') {
  ...
  $smpltmp = xl_list_label($abnQry['sample']);
		...
		?>
		<!--/*echo"<td>";
		echo "<select name='abn[]' style='background-color:#DCDCDC;'>";
		...
		echo "</td>";
		 ...
		 <td> 
        <select name='abn[]' style='background-color:#DCDCDC;'>
		<option value='no' <?php if ($abntmp=='no') echo 'selected="selected"'; ?> >No</option>
		<option value='yes' <?php if ($abntmp=='yes') echo 'selected="selected"'; ?>>Yes</option>
		</select>
		</td>
       ...
       // echo "</td>\n";
        ...
        if ($result_document_id) {
          ...
          echo "  <td><input type='text' size='2' name='val[]' value='$result_result' style='background-color:	#DCDCDC;'></td>";
		...
		?>
        <td> 
        <select name='sample[]' style='background-color:#DCDCDC;'>
		<option value='no' <?php if ($smpltmp=='no') echo 'selected="selected"'; ?> >Not Sent</option>
		<option value='yes' <?php if ($smpltmp=='yes') echo 'selected="selected"'; ?>>Sent</option>
		</select>
		</td>
		<?php
		
       // echo myCellText($result_id);
       // echo "</td>\n";

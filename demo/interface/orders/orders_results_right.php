...
<body class="body_top">
<form method='post' action='orders_results_right.php?batch=<?php echo $form_batch; ?>&review=<?php echo $form_review; ?>'
...
<?php
$order=sqlStatement("SELECT * from procedure_order where procedure_order_id='".$orderid."'");
   $order1=sqlFetchArray($order);
  /*  if($order1['order_status']!="received")
	{
		$address = "{$GLOBALS['rootdir']}/main/finder/p_dynamic_finder_lab.php";
       $message="The Sample is not yet Received";
...
if ($form_batch) {
  ...
  "FROM procedure_order AS po " .
  "$joins " .
  "WHERE po.patient_id = '$pid' AND po.procedure_order_id='$orderid' AND $where " .
  "ORDER BY $orderby";
}
...
while ($row = sqlFetchArray($res)) {
	$order=sqlStatement("SELECT * from procedure_sample where procedure_order_id='".$row['procedure_order_id']."'");
   $order1=sqlFetchArray($order);
  ...
  $date_collected = empty($row['date_collected'  ]) ? $order1['sample_date_collected'] : substr($row['date_collected'], 0, 16);
  $specimen_num   = empty($row['specimen_num'    ]) ? $order1['specimen'] : $row['specimen_num'];

...
if (!$alertmsg && ($_POST['bn_save'] || $_POST['bn_save_close'])) {
  ...
  for ($lino = 1; $bill["$lino"]['code_type']; ++$lino) {
    ...
    // Otherwise it's a new item...
	  ...
	  $re=sqlQuery("select * from drugs where batch=?",array($notecodes));
	  $drug_id=$re['drug_id'];
	  $prescription_id=111;
	  $inventory_id=111;
...
$billresult = getBillingByEncounterPharm($pid, $encounter, "*");
?>
<html>
<head>
...
<body class="body_top">
<form method="post" action="<?php echo $rootdir; ?>/forms/fee_sheet_ph/new.php"
...
//Lab Test
 /*  ++$i;
  ...
  $tres = sqlStatement(" 
  SELECT c.Service_Id,d.drug_id,d.batch batch,c.code,c.code_text,c.code_type,sum(e.quantity ) sumq
    FROM codes AS c,drugs d,drug_sales e WHERE c.code=d.name and
    c.active = 1 AND c.code_type=11 and d.drug_id=e.drug_id and d.totalStock>0
    group by e.drug_id
      ORDER BY c.code");
  while ($trow = sqlFetchArray($tres)) {
	  if($trow['sumq']!=0){
    echo "    <option value='Pharmacy Charge|" . attr($trow['code']) . '|' . attr($trow['code_text']) . "'>" .
      text($trow['code'].'-'.$trow['batch']);
    //if ($trow['name'] !== $trow['selector']) echo ' ' . text($trow['name']);
    echo "</option>\n";
	  }
  }
...
<?php
  foreach ($nofs_code_types as $key => $value) {
	  if($value['label']=='Pharmacy Charge'){
    echo "   <option value='" . attr($key) . "'";
...
<table cellspacing='5'>
 <tr>
  ...
  <td class='billcell'><b><?php echo xlt('Medicine');?></b></td>
<?php if (modifiers_are_used(true)) { ?>
  <td class='billcell'><b><?php echo xlt('');?></b></td>
<?php } ?>
<?php if (fees_are_used()) { ?>
  <td class='billcell' align='right'><b><?php echo xlt('Price');?></b>&nbsp;</td>
  <td class='billcell' align='center'><b><?php echo xlt('Quantity');?></b></td>
<?php } ?>
...
<?php } ?>
  ...
  <td class='billcell' align='center'<?php echo $usbillstyle; ?>><b><?php echo xlt('Batch Codes');?></b></td>

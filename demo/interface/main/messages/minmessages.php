...
if ($show_all=='yes') {
    $showall = "yes";
    $lnkvar="'\kavaii\interface\main\messages\minmessages.php?show_all=no&$activity_string_html' name='Just Mine' onclick=\"top.restoreSession()\"> (".htmlspecialchars( xl('Just Mine'), ENT_NOQUOTES).")";
}
else {
    $showall = "no";
    $lnkvar="'\kavaii\interface\main\messages\minmessages.php?show_all=yes&$activity_string_html' name='See All' onclick=\"top.restoreSession()\"> (".htmlspecialchars( xl('See All'), ENT_NOQUOTES).")";
}
}
?>
<br>
...
if (empty($task) || $task=="add" || $task=="delete") { ?>
  ...
  <?php } else { ?>
    <a href="\kavaii\interface\main\messages\minmessages.php" class="link" onclick="top.restoreSession()"><span><?php echo xlt('Show All'); ?></span></a>
  <?php } ?>
  |
  <?php if ($active == '1') { ?>
    <span><?php echo xlt('Show Active'); ?></span>
  <?php } else { ?>
    <a href="\kavaii\interface\main\messages\minmessages.php?form_active=1" class="link" onclick="top.restoreSession()"><span><?php echo xlt('Show Active'); ?></span></a>
  <?php } ?>
  |
  <?php if ($active == '0') { ?>
    <span><?php echo xlt('Show Inactive'); ?></span>
  <?php } else { ?>
    <a href="\kavaii\interface\main\messages\minmessages.php?form_inactive=1" class="link" onclick="top.restoreSession()"><span><?php echo xlt('Show Inactive'); ?></span></a>
  <?php } ?>
<?php } ?>
...
echo "
<form name=new_note id=new_note action=\"\kavaii\interface\main\messages\minmessages.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&$activity_string_html\" method=post>
...
<table border='0' cellspacing='8'>
 ...
 if ($reply_to) {
  $prow = sqlQuery("SELECT lname, fname " .
   "FROM patient_data WHERE pid = ?", array($reply_to) );
  $patientname = $prow['fname'] . " " . $prow['mname']. " " .$prow['lname'];
 }
   if ($patientname == '') {
...
$ures = sqlStatement("SELECT username, fname, lname FROM users " .
 ...
 while ($urow = sqlFetchArray($ures)) {
  ...
  echo ">" . htmlspecialchars( $urow['username'], ENT_NOQUOTES);
  //if ($urow['lname']) echo " " . htmlspecialchars( $urow['lname'], ENT_NOQUOTES);
  echo "</option>\n";
 }
...
<?php } ?>
 ...
 function sel_patient() {
  ...
  function addtolist(sel){
    ...
    $sort = array("users.lname", "patient_data.lname", "pnotes.title", "pnotes.date", "pnotes.message_status","pnotes.pid");
    ...
    for($i = 0; $i < count($sort); $i++) {
        $sortlink[$i] = "<a href=\"\kavaii\interface\main\messages\minmessages.php?show_all=".attr($showall)."&sortby=".attr($sort[$i])."&sortorder=asc&$activity_string_html\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Up'), ENT_QUOTES)."\"></a>";
    }
    for($i = 0; $i < count($sort); $i++) {
        if($sortby == $sort[$i]) {
            switch($sortorder) {
                case "asc"      : $sortlink[$i] = "<a href=\"minmessages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=desc&$activity_string_html\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortup.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Up'), ENT_QUOTES)."\"></a>"; break;
                case "desc"     : $sortlink[$i] = "<a href=\"minmessages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=asc&$activity_string_html\" onclick=\"top.restoreSession()\"><img src=\"../../../images/sortdown.gif\" border=0 alt=\"".htmlspecialchars( xl('Sort Down'), ENT_QUOTES)."\"></a>"; break;
            } break;
        }
    }
    ...
    if($prev >= 0) {
        $prevlink = "<a href=\"\kavaii\interface\main\messages\minmessages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($prev)."&$activity_string_html\" onclick=\"top.restoreSession()\"><<</a>";
    }
    else {
        $prevlink = "<<";
    }
    ...
    if($next < $total) {
        $nextlink = "<a href=\"\kavaii\interface\main\messages\minmessages.php?show_all=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($next)."&$activity_string_html\" onclick=\"top.restoreSession()\">>></a>";
    }
    else {
        $nextlink = ">>";
    }
    ...
    <form name=MessageList action=\"\kavaii\interface\main\messages\minmessages.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&$activity_string_html\" method=post>
    <input type=hidden name=task value=delete>
        <tr height=\"24\" style=\"background:lightgrey\">
            ...
            <td width=\"15%\" style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid; \" class=bold>&nbsp;<b>" .
              ...
              htmlspecialchars( xl('MED No.'), ENT_NOQUOTES) . "</b> $sortlink[5]</td>
        </tr>";
        ...
        while ($myrow = sqlFetchArray($result)) {
            ...
            if ($myrow['users_lname']) {
                $name .= " " . $myrow['users_lname'];
            }
            $patient = $myrow['pid'];
			$med=sqlStatement("SELECT genericname1 from patient_data where pid='".$patient."'");
			$med1=sqlFetchArray($med);
            if ($patient>0) {
            ...
            <tr id=\"row$count\" style=\"background:white\" height=\"24\">
	          ...
	          htmlspecialchars( $name, ENT_NOQUOTES) . "</td><td width=5></td></tr></table></td>
                <td style=\"border-bottom: 1px #000000 solid; border-right: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\"><a href=\"\kavaii\interface\main\messages\minmessages.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&task=edit&noteid=" .
		  ...
		  htmlspecialchars( $patient, ENT_NOQUOTES) . "</a></td><td width=5></td></tr></table></td>
	          ...
	          htmlspecialchars( $myrow['message_status'], ENT_NOQUOTES) . "</td><td width=5></td></tr></table></td>
			   <td style=\"border-bottom: 1px #000000 solid;\"><table cellspacing=0 cellpadding=0 width=100%><tr><td width=5></td><td class=\"text\"><a href=\"../../patient_file/summary/demographics.php?set_pid=".attr($myrow['pid'])."\" onclick=\"top.restoreSession()\">" .
	          htmlspecialchars( $med1['genericname1'], ENT_NOQUOTES) . "</td><td width=5></td></tr></table></td>
            </tr>";
        }
    ...
    <table border=0 cellpadding=5 cellspacing=0 width=90%>
        <tr>
            <td class=\"text\"><a href=\"\kavaii\interface\main\messages\minmessages.php?showall=".attr($showall)."&sortby=".attr($sortby)."&sortorder=".attr($sortorder)."&begin=".attr($begin)."&task=addnew&$activity_string_html\" onclick=\"top.restoreSession()\">" .

    ...
    $iSortCol = intval($_GET["iSortCol_$i"]);
      ...
      if ($aColumns[$iSortCol] == 'name') {
	   $orderby .= "status,genericname1";
        ...
        $orderby .= "`" . escape_sql_column_name($aColumns[$iSortCol],array('patient_data','form_encounter','procedure_order','billing')) . "` $sSortDir";
      }
		}
	}
}
...
if (isset($_GET['sSearch']) && $_GET['sSearch'] !== "") {
  ...
  foreach ($aColumns as $colname) {
    ...
    if ($colname == 'name') {
      ...
      $where .= "`" . escape_sql_column_name($colname,array('patient_data','form_encounter','procedure_order','billing')) . "` LIKE '$sSearch%' ";
    }
  }
  if ($where) $where .= ")";
}
...
//
$sellist = 'a.pid,a.encounter,c.order_status,c.procedure_order_id';
...
foreach ($aColumns as $colname) {
  ...
  if ($colname == 'name') {
    ...
    $sellist .= "`" . escape_sql_column_name($colname,array('patient_data','form_encounter','procedure_order','billing')) . "`";
  }
}
// Get total number of rows in the table.
//

$row = sqlQuery("SELECT COUNT(distinct(c.procedure_order_id)) AS count FROM form_encounter a,patient_data b,procedure_order  c where a.pid=b.pid and a.encounter=c.encounter_id and c.order_status IN ('pending','collected','received')");

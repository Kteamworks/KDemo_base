...
{
$row = sqlQuery("SELECT COUNT(b.id) AS count FROM form_encounter a,patient_data b where a.pid=b.pid and a.provider_id='".$providerid."' and a.out_to='Sent To' and a.date='".$today."'");
}
...
if($row2["newcrop_user_role"]=="erxnurse")
{
$query ="SELECT $sellist FROM form_encounter a,patient_data b where a.pid=b.pid and a.provider_id IN ($X)and a.date='".$today."' order by $orderby, encounter desc  $limit";
}
else 
{
$query = "SELECT $sellist FROM patient_data a,form_encounter b where a.pid=b.pid and b.provider_id='".$providerid."'  and b.out_to='Sent To' and b.date='".$today."' order by $orderby,out_time asc  $limit";
}

    ...
    $iSortCol = intval($_GET["iSortCol_$i"]);
      ...
      if ($aColumns[$iSortCol] == 'name') {
	   $orderby .= "genericname1";
        ...
        $orderby .= "`" . escape_sql_column_name($aColumns[$iSortCol],array('patient_data','form_encounter','t_form_ot','billing')) . "` $sSortDir";
      }
		}
	}
}
...
if (isset($_GET['sSearch']) && $_GET['sSearch'] !== "") {
  ...
  foreach ($aColumns as $colname) {
    ...
    if ($colname == 'name') {
      ...
      $where .= "`" . escape_sql_column_name($colname,array('patient_data','form_encounter','t_form_ot','billing')) . "` LIKE '$sSearch%' ";
    }
  }
  if ($where) $where .= ")";
}
...
foreach ($aColumns as $colname) {
  ...
  if ($colname == 'name') {
    ...
    $sellist .= "`" . escape_sql_column_name($colname,array('patient_data','form_encounter','t_form_ot','billing')) . "`";
  }
}
// Get total number of rows in the table.
//

$row = sqlQuery("SELECT COUNT(distinct(a.encounter)) AS count FROM form_encounter a,patient_data b,t_form_ot c where a.pid=b.pid and a.encounter=c.encounter and a.pc_catid=12 and c.status=1 ");

...
$res = sqlStatement("SELECT option_id, title FROM list_options WHERE " .
  "list_id = 'Lab_Report_Specific' ORDER BY seq, title");
...
$(document).ready(function() {
 ...
 var oTable = $('#pt_table').dataTable( {
  ...
  "bServerSide": true,
  "sAjaxSource": "p_dynamic_finder_ajax_lab_report_review.php",
...
<?php } ?>
 ...
 $(document).ready(function() {
    $('ul.menu1 li').live("click", function() {
		var name=$(this).text();
		var review=1;
		if(name=="View Report Review")
		{
		top.restoreSession();
		document.location.href="../../patient_file/summary/labdata_review.php?set_pid=" + newpid+'&encounter='+enc+'&orderid='+orderid;
		}else
		if(name=="View Report")
		{
		top.restoreSession();
		document.location.href="../../patient_file/summary/labdata.php?set_pid=" + newpid+'&encounter='+enc+'&orderid='+orderid;
		}else
		if(name=="Collected")
		{
		//alert(newpid);
...
function showUser(str) {
   ...
   /* if (str == "") {
        ...
        xmlhttp.open("GET","update_pending.php?q="+str,true);
        xmlhttp.send();
    }*/
}
...
<div class="menu">
	<ul class="menu1">
	    <li><a href="#">View Report Review</a></li>

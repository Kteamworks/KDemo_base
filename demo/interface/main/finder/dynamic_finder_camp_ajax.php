...
if (isset($_GET['iSortCol_0'])) {
    ...
    $iSortCol = intval($_GET["iSortCol_$i"]);
      ...
      if ($aColumns[$iSortCol] == 'name') {
        ...
        $orderby .= "`" . escape_sql_column_name($aColumns[$iSortCol],array('patient_camp','form_encounter')) . "` $sSortDir";
      }
		}
	}
}
...
if (isset($_GET['sSearch']) && $_GET['sSearch'] !== "") {
  ...
  foreach ($aColumns as $colname) {
    ...
    if ($colname == 'name') {
      ...
      $where .= "`" . escape_sql_column_name($colname,array('patient_camp','form_encounter')) . "` LIKE '$sSearch%' ";
    }
  }
  if ($where) $where .= ")";
}
...
foreach ($aColumns as $colname) {
  ...
  if ($colname == 'name') {
    ...
    $sellist .= "`" . escape_sql_column_name($colname,array('patient_camp','form_encounter',)) . "`";
  }
}

// Get total number of rows in the table.
//
$row = sqlQuery("SELECT COUNT(id) AS count FROM patient_camp ");
$iTotal = $row['count'];

// Get total number of rows in the table after filtering.
//
$row = sqlQuery("SELECT COUNT(id) AS count FROM patient_camp $where");
...
$query = "SELECT $sellist FROM patient_camp  $where  $orderby  $limit ";

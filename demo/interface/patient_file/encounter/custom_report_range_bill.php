    ...
    foreach($newpatient as $patient){
        ...
        */
        ...
        //print "<h1>".xlt('Patient Data').":</h1>";
               $patdata = getPatientData($pids[$iCounter], 'rateplan,date,sex,DOB,genericname1,fname,mname,lname,pubpid,street,city,state,postal_code,providerID');
...
*/
	...
	echo "<tr><td style='padding-right: 100px;'  >" . xlt('Doctor') . ":&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp &nbsp&nbsp" . text($test) . "  " . text($row2['lname']) . "</td>";
	echo "<td  style='padding-right: 10px;' >" . xlt('ADM Date') . ": ". text(date('d/M/y',strtotime($admit1['admit_date'])))."</td></tr>";
	echo "<tr><td  style='padding-right: 100px;' >" . xlt('Department') . ": " . text($row2['specialty']) . "</td>";
	echo "<td style='padding-right: 10px;'  >" . xlt('Ward/Bed') . ":&nbsp " . text($admit1['admit_to_ward']) ." , ".text($admit1['admit_to_bed']). "</td></tr>";

	echo "<tr><td  style='padding-right: 100px;' >" . xlt('Rate Plan') . ": &nbsp&nbsp&nbsp" . text($patdata['rateplan']) . "</td>";
	echo "<td   style='padding-right: 10px;'>" . xlt('DCH Date') . ":&nbsp". text(date(' d/M/y',strtotime($admit1['discharge_date'])))."</td></tr>";
        ...
        if (count($patient) > 0) {
            ...
            //foreach ($patient as $be) {
				...
				foreach ($billing as $b)				
				{
					...
					$counta=sqlStatement("SELECT sum(fee) as st ,code_type, count(code_type) as c from billing where activity='1' and encounter='".$encounter."' and code_type='".$ct."'");
...
//		   echo "<tr style='border-top: 1px solid #000;'><td colspan=6>&nbsp; </td></tr>";
	...
	echo "<tr style='border-top: 1px solid #000;' ><td></td><td></td><td></td><td></td><td class='bold' style='border-bottom: 1px solid #000;' style='text-align:right' nowrap>".xlt('Net Payments')."</td><td class='text' style='border-bottom: 1px solid #000;' >" ."". oeFormatMoney($nettotal) . "</td></tr>";
     echo "<tr><td></td><td></td><td></td><td></td><td class='bold' style='border-bottom: 0px solid #000;'  style='text-align:right' nowrap>".xlt('Total Bill Amount')."</td><td class='text' style='border-bottom: 0px solid #000;'>" . oeFormatMoney($total) . "</td></tr>";
    ...
    echo "<tr><td></td><td></td><td></td><td></td><td class='bold' style='border-bottom: 1px solid #000;' style='text-align:right' nowrap>".xlt('Balance Due')."</td><td class='text' style='border-bottom: 1px solid #000;'>" . oeFormatMoney($total-$nettotal-$totaldis) . "</td></tr>";
	echo "</table>";
	   }
        echo "</div>";
 ...
 <tr>
  ...
  <td align='left' valign='middle' height="100%">
	<table style='border-left:1px solid; width:100%; height:100%' >
		<tr>
		<td>
		<select name='form_billlist'>
		<?php 
		
  

		$enc=$_SESSION['encounter'];
		$billqry=sqlStatement("select distinct partial_bill_id from billing where encounter='".$enc."'");
		//$billres=sqlFetchArray($billqry);
		while($billres=sqlFetchArray($billqry))
		{
			$billres['partial_bill_id']='SBL0012';
			$rdm=$billres['partial_bill_id'];
		 echo "<option value=\"$rdm\">".$billres['partial_bill_id']." </option>";

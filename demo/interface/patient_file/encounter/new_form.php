...
$gender=getPatientData($pid, "sex");
$gender=$gender['sex'];  
if (!empty($reg)) {
  $StringEcho= '<ul id="sddm">';
  if(isset($hide)){
    $StringEcho.= "<li><a id='enc2' >" . htmlspecialchars( xl('Visit Summary'),ENT_NOQUOTES) . "</a></li>";
  }else{
    $StringEcho.= "<li><a href='JavaScript:void(0);' id='enc2' onclick=\" return top.window.parent.left_nav.loadFrame2('enc2','RBot','patient_file/encounter/encounter_top.php')\">" . htmlspecialchars( xl('Visit Summary'),ENT_NOQUOTES) . "</a></li>";
  }
  $flag=1;
  $flag2=1;
  foreach ($reg as $entry) {
    ...
    if ($old_category != $new_category) {
      ...
      if ($old_category != '') {
		  //$obsfixedcategory="Obstetric";
		  //$StringEcho.= "<li><a href='JavaScript:void(0);' onClick=\"mopen('$DivId');\" >$obsfixedcategory</a><div id='$DivId' ><table border='0' cellspacing='0' cellpadding='0'>";
	...
	$sqql="select pc_catid, visittype from form_encounter where encounter='".$enqc."'";
	...
	$visttype=$rsqql['visittype'];
	if($catg!=17 && $new_category=="Obstetric" )  { continue; }
    if ($nickname=='General IP Admission')
    	{
	...
	else
		{
				...
				if($gender!="Female" && $new_category=='Obstetric')
				{
					continue;
				}
				$StringEcho.= "<tr><td style='border-top: 1px solid #000000;padding:0px;'><a onclick=\"openNewForm('" . $rootdir .'/patient_file/encounter/load_form.php?formname=' .urlencode($entry['directory']) .
	...
	if($catg==17 && $flag==1 && $flag2==1 && $gender=="Female")
	{
					if($new_category=='Obstetric')
					{
					$OBS="OBS Data";
					$StringEcho.= "<tr><td style='border-top: 1px solid #000000;padding:0px;'><a onclick=\"openNewForm('" . $rootdir .'/patient_file/OBSpecific/history.php'.
                    "')\" href='JavaScript:void(0);'>" . xl_form_title($OBS) . "</a></td></tr>";
					$OBSDS="Discharge Summary";
					$StringEcho.= "<tr><td style='border-top: 1px solid #000000;padding:0px;'><a onclick=\"openNewForm('" . $rootdir .'/patient_file/reportobs/patient_report.php'.
                    "')\" href='JavaScript:void(0);'>" . xl_form_title($OBSDS) . "</a></td></tr>";
					$flag=2;
					$flag2=2;
					}
	}		

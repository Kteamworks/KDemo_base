...
<table border='0' cellpadding='0' cellspacing='0'>
 ...
 <tr>
  ...
  <td>
   ...
   <?php if ($mode != "modify") { ?>
     <select name="code_type" onchange='doc(this.value)'>
   <?php } ?>
  ...
  <input type='text'  id='code' size='6' name='code'  value='<?php echo attr($code) ?>'
     ...
     />
 <select name='code1' id='code1' style='display:none;' onchange='doctor(this.value)'>
<?php
$ores = sqlStatement("SELECT user_id, username FROM users " .
  "WHERE authorized=1 ORDER BY user_id");
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['user_id'] . "'";
  if ($orow['user_id'] == $code) echo " selected";
  echo ">" . $orow['username'] . "</option>\n";
}
?>
   </select>
   <?php } ?>

...
if ($_POST['form_save']) {
  ...
  $form_birth = fixDate($_POST['form_birth'], '');
  $form_birth_time = fixDate($_POST['form_birth_time'], '');
  $form_birth_sec = fixDate($_POST['form_birth_sec'], '');
  $form_birth_time_sec = fixDate($_POST['form_birth_time_sec'], '');
  ...
  if ($issue) {
   ...
   $query = "UPDATE lists SET " .
	...
	"birthdate = "      . QuotedOrNull($form_birth)   . ", "  .
    "birthdate_sec = "      . QuotedOrNull($form_birth_sec)     . ", "  .
	//"birthtime = '"    . add_escape_custom(date('H:i:s', strtotime($_POST['form_birth_time'])))  . "', "  .
	//"birthtime_sec = '"    . add_escape_custom(date('H:i:s', strtotime($_POST['form_birth_time_sec'])))  . "', "  .
	"birthtime = "    . QuotedOrNull($_POST['form_birth_time'])  . ", "  .
   "birthtime_sec = "    . QuotedOrNull($_POST['form_birth_time_sec'])  . ", "  .
    ...
    "occurrence = '"  . add_escape_custom($_POST['form_occur'])        . "', " .
	 "sex = '"  . add_escape_custom($_POST['form_sex'])        . "', " .
	  "sex_sec = '"  . add_escape_custom($_POST['form_sex_sec'])        . "', " .
    ...
    if($_POST['form_diagnosis']=="ICD9:V27.2")
	{
   $issue = sqlInsert("INSERT INTO lists ( " .
    "date, pid, type, title, activity, comments, begdate, enddate, birthdate,birthtime,birthdate_sec,birthtime_sec,returndate, " .
    "diagnosis, occurrence, sex,sex_sec,classification, referredby, user, groupname, " .
    ...
    QuotedOrNull($form_end)        . ", "  .
	  QuotedOrNull($form_birth)        . ", "  .
	    QuotedOrNull($_POST['form_birth_time'])        . ", "  .
	//"'".add_escape_custom(date('H:i:s', strtotime($_POST['form_birth_time']))). "', "  .
	QuotedOrNull($form_birth_sec)        . ", "  .
	  QuotedOrNull($_POST['form_birth_time_sec'])        . ", "  .
	//"'".add_escape_custom(date('H:i:s', strtotime($_POST['form_birth_time_sec']))). "', "  .
    QuotedOrNull($form_return)       . ", "  .
    "'" . add_escape_custom($_POST['form_diagnosis'])   . "', " .
    "'" . add_escape_custom($_POST['form_occur'])       . "', " .
	"'" . add_escape_custom($_POST['form_sex'])       . "', " .
	"'" . add_escape_custom($_POST['form_sex_sec'])       . "', " .
    ...
    "'" . add_escape_custom($form_injury_type)          . "', " .
	 "'" . add_escape_custom($encounter) . "', " .
    "'" . add_escape_custom($_POST['form_reaction'])         . "' " .
   ")");
    }else
	{
		 $issue = sqlInsert("INSERT INTO lists ( " .
    "date, pid, type, title, activity, comments, begdate, enddate, birthdate,birthtime,returndate, " .
    "diagnosis, occurrence, sex,classification, referredby, user, groupname, " .
    "outcome, destination, reinjury_id, injury_grade, injury_part, injury_type, " .
    "encounter,reaction " .
    ") VALUES ( " .
    "NOW(), " .
...
<?php require($GLOBALS['srcdir'] . "/restoreSession.php"); ?>
 ...
 function newtype(index) {
  ...
  var datedisp = (index == <?php echo issueTypeIndex('delivery'); ?>) ? '' : 'none';
  var enddisp = (index == <?php echo issueTypeIndex('delivery'); ?>) ? 'none' : '';
  var startdisp = (index == <?php echo issueTypeIndex('delivery'); ?>) ? 'none' : '';
  var disp= document.getElementById('form_diagnosis');
  //var disp =(index == 'ICD9:V27.2') ? 'none' : ' ';
  ...
  document.getElementById('row_occurrence'    ).style.display = enddisp;
  document.getElementById('row_startdate'     ).style.display = startdisp;
  ...
  document.getElementById('row_birthdate'      ).style.display = datedisp;
   document.getElementById('row_birthdate_time'      ).style.display = datedisp;
  document.getElementById('row_enddate'      ).style.display = enddisp;
  document.getElementById('row_sex'      ).style.display = datedisp;
  //document.getElementById('row_birthdate_sec'      ).style.display = disp.value =='V27.2' ? '' : 'none';
   document.getElementById('row_birthdate_sec'      ).style.display = datedisp;
	document.getElementById('row_birthdate_time_sec'      ).style.display = datedisp;
  document.getElementById('row_sex_sec'      ).style.display = datedisp;
  document.getElementById('row_outcome'    ).style.display = enddisp;
  document.getElementById('row_destination'    ).style.display = enddisp;
...
function getDate(secondDate)
{
	document.getElementById('form_birth_sec').value=secondDate;  
	
}
function getTime(secondTime)
{
	document.getElementById('form_birth_time_sec').value=secondTime;  
 ...
 <tr id='row_diagnosis'>
  ...
  <td>
   <input type='text' size='50' name='form_diagnosis'
    value='<?php echo attr($irow['diagnosis']) ?>' onclick='sel_diagnosis();newtype($index)' 
...
<tr id='row_birthdate'>
  <td valign='top' nowrap><b><?php echo xlt('Birth Date'); ?>:</b></td>
  <td>

   <input type='text' size='10' name='form_birth' id='form_birth'
    value='<?php echo attr($irow['birthdate']) ?>'  
    onkeyup='datekeyup(this,mypcc)' onchange="getDate(this.value)"  onblur='dateblur(this,mypcc)'
    title='<?php echo xla('yyyy-mm-dd date of birth'); ?>' />
   <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22'
    id='img_birth' border='0' alt='[?]' style='cursor:pointer'
    title='<?php echo xla('Click here to choose a date'); ?>' />
  </td>
 </tr>
 <tr id='row_birthdate_time'>
  <td valign='top' nowrap><b><?php echo xlt('Birth Time'); ?>:</b></td>
  <td>

   <input type='text' size='10' name='form_birth_time' id='form_birth_time'
    value='<?php echo attr($irow['birthtime']) ?>'
    onkeyup='datekeyup(this,mypcc)' onchange="getTime(this.value)" onblur='dateblur(this,mypcc)'
    title='<?php echo xla('yyyy-mm-dd date of birth'); ?>' />
   <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22'
    id='img_birth_time' border='0' alt='[?]' style='cursor:pointer'
    title='<?php echo xla('Click here to choose a time '); ?>' />
  </td>
 </tr>
  <tr id='row_sex'>
  <td valign='top' nowrap><b><?php echo xlt('Gender of Baby'); ?>:</b></td>
  <td>
   <?php

    generate_form_field(array('data_type'=>1,'field_id'=>'sex','list_id'=>'sex','empty_title'=>'SKIP'), $irow['sex']);
   ?>
  </td>
 </tr>

 <tbody id="check" style="display:none">
 <tr id='row_birthdate_sec' >
 
  <td valign='top' nowrap><b><?php echo xlt('Birth Date(2nd Baby)'); ?>:</b></td>
  <td>

   <input type='text' size='10' name='form_birth_sec' id='form_birth_sec'
    value='<?php echo attr($irow['birthdate_sec']) ?>'
    onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)'
    title='<?php echo xla('yyyy-mm-dd date of birth'); ?>'/>
   <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22'
    id='img_birth_sec' border='0' alt='[?]' style='cursor:pointer'
    title='<?php echo xla('Click here to choose a date'); ?>' />
  </td>
 </tr>
  
 <tr id='row_birthdate_time_sec'>
  <td valign='top' nowrap><b><?php echo xlt('Birth Time(2nd Baby)'); ?>:</b></td>
  <td>

   <input type='text' size='10' name='form_birth_time_sec' id='form_birth_time_sec'
    value='<?php echo attr($irow['birthtime_sec']) ?>'
    onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)'
    title='<?php echo xla('yyyy-mm-dd date of birth'); ?>' />
   <img src='../../pic/show_calendar.gif' align='absbottom' width='24' height='22'
    id='img_birth_time_sec' border='0' alt='[?]' style='cursor:pointer'
    title='<?php echo xla('Click here to choose a time'); ?>' />
  </td>
 </tr>

  <tr id='row_sex_sec'>
  <td valign='top' nowrap><b><?php echo xlt('Gender of Baby(2nd Baby)'); ?>:</b></td>
  <td>
   <?php

    generate_form_field(array('data_type'=>1,'field_id'=>'sex_sec','list_id'=>'sex','empty_title'=>'None'), $irow['sex_sec']);
   ?>
  </td>
 </tr>
 </tbody>
...
?>
 ...
 <tr id='row_outcome' <?php if ($GLOBALS['athletic_team'] || $GLOBALS['ippf_specific']) echo " style='display:none;'"; ?>>
 ...
 <tr id='row_destination'<?php if ($GLOBALS['athletic_team'] || $GLOBALS['ippf_specific']) echo " style='display:none;'"; ?>>
...
<script language='JavaScript'>
 ...
 Calendar.setup({inputField:"form_birth", ifFormat:"%Y-%m-%d", button:"img_birth"});
 Calendar.setup({inputField:"form_birth_time", ifFormat:"%H:%M:%S", button:"img_birth_time",showsTime:'true'});
 Calendar.setup({inputField:"form_birth_time_sec", ifFormat:"%H:%M:%S", button:"img_birth_time_sec",showsTime:'true'});
 Calendar.setup({inputField:"form_birth_sec", ifFormat:"%Y-%m-%d", button:"img_birth_sec"});

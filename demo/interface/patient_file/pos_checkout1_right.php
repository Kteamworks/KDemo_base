...
$details = empty($_GET['details']) ? 1 : 0;

$patient_id = empty($_GET['ptid']) ? $pid : 0 + $_GET['ptid'];

// Get the patient's name and chart number.
$patdata = getPatientData($patient_id, 'sex,DOB,genericname1,date,fname,mname,lname,pubpid,genericname1,DOB,street,city,state,postal_code');
...
//
$prevsvcdate = '';
function receiptDetailLine($svcdate, $itemno,$description, $amount, $quantity) {
...
//
function receiptPaymentLine($paydate, $amount,$paymode, $description='') {
...
function generate_receipt($patient_id, $encounter=0) {
  ...
  $patdata = getPatientData($patient_id, 'sex,DOB,genericname1,date,fname,mname,lname,pubpid,street,city,state,postal_code,providerID');
...
$row2=  sqlFetchArray($row1);
   ...
   echo "<tr><td>" . xlt('Registration Date') . ":&nbsp&nbsp ". text(date(' d/M/y',strtotime($patdata['date'])))."</td>";
	echo "<td>" . xlt('Bill Date') . ":&nbsp&nbsp ". text(date('d/M/y'))."</td></tr>";
	echo "<tr><td>" . xlt('Patient') . ": <b>&nbsp&nbsp" . text($patdata['fname']) . "  " . text($patdata['lname']) . "</b></td>";
	echo "<td>" . xlt('Patient ID') . ": &nbsp&nbsp" . text($patdata['genericname1']) . "</td></tr>";
	echo "<tr><td>" . xlt('Age/Gender') . ": &nbsp&nbsp" . ageCalculator($dob) ." , ".text($patdata['sex']). "</td>";
    echo "<td>" . xlt('Doctor') . ": &nbsp&nbsp" . text($row2['fname']) . "  " . text($row2['lname']) . "</td></tr>";
	echo "<tr><td>" . xlt('Department') . ": &nbsp&nbsp" . text($row2['specialty']) . "</td>";
	echo "<td>" . xlt('Encounter') . ":&nbsp&nbsp " . text($encounter) . "</td></tr>";
...
<?php
  ...
  if ($INTEGRATED_AR) {
    ...
    while ($inrow = sqlFetchArray($inres)) {
      $charges += sprintf('%01.2f', $inrow['fee']);
      //receiptDetailLine($svcdate, $inrow['id'],$inrow['code_text'],
...
<?php
  if ($INTEGRATED_AR) {
    ...
    // Get other payments.
	//$inres1=sqlStatement("select * from payments where encounter=? and pid=? and method='refund cash'",array($patient_id,$encounter));
    $inres = sqlStatement("SELECT " .
      "a.code_type, a.code, a.modifier, a.memo, a.payer_type, a.adj_amount, a.pay_amount, " .
    ...
    while ($inrow = sqlFetchArray($inres)) {
     ...
     // receiptPaymentLine($svcdate, $inrow['pay_amount'],$inrow['payment_method'],
      //  $payer . ' ' . $inrow['reference']);
    }
	//while ($inrow1= sqlFetchArray($inres1)) {
      //$payer = empty($inrow['payer_type']) ? 'Pt' : ('Ins' . $inrow['payer_type']);
      //$charges2= sprintf('%01.2f', $inrow['amount1']);
     // receiptPaymentLine($svcdate, $inrow['pay_amount'],$inrow['payment_method'],
     //  $payer . ' ' . $inrow['reference']);
	 	...
	 	/*	 
             ...
             */
			 ...
			 $a=-($_POST['form_discount']);
			$inres1 = sqlStatement("SELECT dtime,amount1,amount2,receipt_id,method FROM payments WHERE " .
            "pid = ? AND encounter = ? and method='refund cash' and amount1='$a' ORDER BY dtime", array($patient_id,$encounter) );
			
		
			$inrow = sqlFetchArray($inres1);
			
			/*while ($inrow = sqlFetchArray($inres1)) {
			//$payer = empty($inrow['payer_type']) ? 'Pt' : ('Ins' . $inrow['payer_type']);
	...
	//$form_pid,$form_encounter,$time ,$_SESSION['authUser'],'Refund Cash','',$amount,0
	
	?>
	 <table border='0' align='center' cellspacing='8'>
 <tr>
  <td><?php echo xlt('Paid Via'); ?>:</td>
  <td><?php echo text('Cash')//echo generate_display_field(array('data_type'=>'1','list_id'=>'payment_method'),$payrow['method']); ?></td>
 </tr>
 <tr>
 ...
 <tr>
  <td><?php echo xlt('Paid Amount'); ?>:</td>
  <td><?php echo text(oeFormatMoney(-($inrow['amount1']))) ?></td>
 </tr>
 
 <tr>
  <td><?php echo xlt('Receipt No'); ?>:</td>
  <td><?php echo text($inrow['receipt_id']) ?></td>
 </tr>
 <tr>
  <td><?php echo xlt('Received By'); ?>:</td>
  <td><?php echo text($_SESSION['authUser']) ?></td>
 </tr>
 </table>
...
?>
 ...
 <tr>
  ...
  <td><b><?php //echo xlt('Balance Due'); echo "   "; echo text(oeFormatMoney($charges, true)); ?></b></td>
...
if ($_POST['form_save']) {
  ...
  for ($lino = 0; $lines[$lino]['code_type']; ++$lino) {
    ...
    if ($code_type == 'PROD') {
      ...
      $query = "UPDATE billing SET fee = ?, billed = 0, " .
  ...
  if ($_POST['form_discount']) {
    ...
    $payment_id = idSqlStatement("insert into ar_session set "    .
					...
					", adjustment_code = 'refund'" .
					", post_to_date = now() " .
					", payment_method = ?",
					array(0,$form_pid,$_SESSION['authUserID'],0,'',$amount,'cash'));
	  ...
	  $query1 = "INSERT INTO payments ( " .
        ...
        ")";
      sqlStatement($query1, array($form_pid,$form_encounter,$time ,$_SESSION['authUser'],'Refund Cash','',$amount,0,3) );
      sqlQuery("Update billing_main_copy set refund_amt=? where encounter=?",array($amount,$encounter));   
	  
	//  sqlInsert("INSERT INTO payments  SET ( "pid, encounter, dtime, user, method, source, amount1, amount2 ") VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)", array($form_pid,$form_encounter,now(),$_SESSION['authId'],'cash','',$amount,0) ); 
    }
    else {
      $msg = invoice_add_line_item($invoice_info, 'REFUND',
        '', $memo, 0 - $amount);
      if ($msg) die($msg);
    }
  }
  ...
  if ($_POST['form_amount']) {
    ...
    if ($INTEGRATED_AR) {
	  ...
	  $insrt_id=idSqlStatement("INSERT INTO ar_activity (pid,encounter,code_type,code,modifier,payer_type,post_time,post_user,session_id,pay_amount,account_code)".
		...
		//$update_id=idSqlStatement("Update payments set amount1=? WHERE pid=? and encounter=?",array($amount,$form_pid,$form_encounter));
		
		
		
							
							/* 		  sqlInsert("INSERT INTO payments  SET ( " .
															"pid, encounter, dtime, user, method, source, amount1, amount2 " .
															") VALUES ( ?, ?, ?, ?, ?, ?, ?, ?)", array($form_pid,$form_encounter,now(),$_SESSION['authId'],'cash','',$amount,0) );  */
...
<form method='post' action='pos_checkout1_right.php' onsubmit="return saveClicked();">
...
<table cellspacing='5'>
 <tr>
  <td colspan='3' align='center'>
   <b><?php echo xlt('Patient Refund for '); ?><?php echo text($patdata['fname']) . " " .
...
<table border='0' cellspacing='4'>
 ...
 <tr>
  <td>
   <?php echo $GLOBALS['discount_by_money'] ? xlt('Refund Amount') : xlt('Refund Percentage'); ?>:
  </td>
  <td>
  ...
  <td>
   <?php echo xlt('Paid By'); ?>:
  </td>
  <td>
   <input type='text' name='form_meth' size='10' value='cash'>
  </td>
 </tr>

 <tr>
  <td>
   <?php echo xlt('Refund Date'); ?>:
  </td>
  <td>

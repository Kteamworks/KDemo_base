...
function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept,$encounter=0,$copay=0,$patcopay=0,$bid) {
  ...
  echo "  <td class='detail' align='right' id='td_charges_$var_index' >" . htmlspecialchars(bucks($charges), ENT_QUOTES) . "</td>\n";
...
function echoLine1($iname, $discount,$date,$sumofchrg,$charges, $ptpaid, $inspaid, $duept,$encounter=0,$copay=0,$patcopay=0,$bid) {
  ...
  $pid=$_SESSION['pid'];
  $claimamt=0;
   $insurance=sqlStatement("SELECT claim_amt, approved_amt,auth_no FROM billing_activity_final where pid = ? and encounter=? ",array($pid,$encounter));
	$ins1 = sqlFetchArray($insurance);
	$claim =$ins1['claim_amt'];
	$approved =$ins1['approved_amt']; 
	$auth_no=$ins1['auth_no'];
  ...
  if($claim==0)
  {	  
  echo "  <td class='detail' align='center' id='td_claim_$var_index' >" . htmlspecialchars(bucks(), ENT_QUOTES) . "<input type='text' name='claimamt'  id='claimamt' " .
  ...
  if($approved==0)
  {
   echo "  <td class='detail' align='center' id='td_apprvl_$var_index' >" . htmlspecialchars(bucks(), ENT_QUOTES) . "<input type='text' name='aprvlname'  id='aprvlname' " .
    " value='" .  '' . "' '  autocomplete='off' " .
   "onkeyup=''  style='width:50px'/></td>\n";
  }
  else
  ...
  if($auth_no==null)
  {
   echo "  <td class='detail' align='center' id='td_authno_$var_index' >" . htmlspecialchars($auth_no, ENT_QUOTES) . "<input type='text' name='auth_no'  id='auth_no' " .
    " value='" .  '' . "' '  autocomplete='off' " .
   "onkeyup=''  style='width:50px'/></td>\n";
  }
  else
  {
	   echo "  <td class='detail' align='center' id='td_authno_$var_index' >" . htmlspecialchars($auth_no, ENT_QUOTES) . "</td>\n";
  }	
  ...
  echo "  <td class='detail' align='center' id='duept_$var_index'>" . htmlspecialchars(bucks(round($duept,2)*1), ENT_QUOTES) . "</td>\n";
  //echo "  <td class='detail' align='center' id='td_payment_$var_index' >" . htmlspecialchars($auth_no, ENT_QUOTES) . "<input type='text' name='payment'  id='payment' " .
    //" value='" .  '' . "' '  autocomplete='off' " .
   //"onkeyup=''  style='width:50px'/></td>\n";
   //"onkeyup=''  style='width:50px'/></td>\n";
  //echo "  <td class='detail' align='right'><input type='text' name='".attr($iname)."'  id='paying_".attr($var_index)."' " .
...
<?php if($_POST['form_method']!='insurance'){ ?>
...
<?php
  ...
  $query = "SELECT fe.encounter, b.id,b.code_type, b.code, b.modifier, b.fee, " .
	...
	"where fe.pid = ?  and b.code not in ('insurance difference amount','insurance copay') and b.encounter='" .$encounter . "'" .
    "ORDER BY b.id";
...
?>
 ...
 <tr bgcolor="#cccccc">
  ...
  <td class="dehead" align="right" >
   <input type='text' align="right" name='f_total' id='f_total' value='<?php echo oeFormatMoney($sumofchrg); ?>' style='width:70px' readonly  />
	
	<input type='hidden' align="right" name='f1_total' id='f1_total' value='<?php echo ($sumofchrg); ?>' />
  </td>
...
<td class="dehead" align="center" width="80" id="td_head_insurance_payment" >
  ...
  <td class="dehead" align="center" width="80" id="td_head_patient_payment" >
   <?php echo htmlspecialchars( xl('Pre Auth No.'), ENT_QUOTES) ?>
  </td>
  <!-- <td class="dehead" align="center" width="80" id="td_head_patient_payment" >
   <?php echo htmlspecialchars( xl('Patient Payment'), ENT_QUOTES) ?>
  </td> -->
  ...
  <td class="dehead" align="center" width="80" id="td_head_patient_payment" >
   <?php //echo htmlspecialchars( xl('Patient Paid'), ENT_QUOTES) ?>
  </td>
 ...
 //------------------------------------------------------------------------------------
	...
	if((($NumberOfInsurance==0 || $value['last_level_closed']==4 || $NumberOfInsurance== $value['last_level_closed'])))
	 {//Patient balance
	  ...
	  "SUM(adj_amount) AS adjustments FROM ar_activity WHERE " .
	  "pid = ? and encounter = ? ",array($pid,$enc));
	   $iamount = sqlQuery("SELECT approved_amt  FROM billing_activity_final WHERE " .
	  "pid = ? and encounter = ? ",array($pid,$enc));

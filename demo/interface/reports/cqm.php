			...
			<td>
				<?php
				 ...
				 $query = "SELECT id, lname, fname FROM users WHERE ".
				  "authorized = 1 $provider_facility_filter ORDER BY fname, lname"; //(CHEMED) facility filter
				 ...
				 while ($urow = sqlFetchArray($ures)) {
				  ...
				  echo ">" . htmlspecialchars( $urow['fname'] . "  " . $urow['lname'], ENT_NOQUOTES) . "\n";
				 }
 ...
 <tr bgcolor='<?php echo $bgcolor ?>'>
  ...
  <?php
   if (isset($row['is_main']) || isset($row['is_sub'])) {
     ...
     echo htmlspecialchars( xl("Provider").": " . $row['prov_fname'] . " " . $row['prov_lname'], ENT_NOQUOTES);

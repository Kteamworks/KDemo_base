...
diagnosis                   = '" . add_escape_custom($_POST["diagnosis"]) ."'";
  ...
  if (empty($id)) {
   ...
   if($rate=="TPAInsurance")
	...
	{
	$discharge_date=date('Y-m-d H:i:s', strtotime($_POST['discharge_date']));
	$admit_date=date('Y-m-d H:i:s', strtotime($_POST['admit_date']));
	$days="SELECT round(TIME_TO_SEC(TIMEDIFF('".$discharge_date."','".$admit_date."'))/86400,0) days";

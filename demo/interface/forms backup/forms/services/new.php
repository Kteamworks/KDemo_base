...
if ($_POST['bn_save'] || $_POST['bn_xmit']) {
  ...
  for ($i = 0; isset($_POST['form_proc_type'][$i]); ++$i) {
	...
	{
	$qgetfee1=sqlStatement("SELECT a.service_id service_id,a.code as cd ,a.code_text as cd_tx,a.code_type as cd_ty,b.pr_price as fee from codes a,prices b  where a.id=b.pr_id and a.code_type=6 and  b.pr_level='HosInsurance' and a.code='".$proc_desc."'");
   ...
   //$fee=$row2['pr_price'];
	...
	{
	$qgetfee=sqlStatement("SELECT a.service_id service_id,a.code as cd ,a.code_text as cd_tx,a.code_type as cd_ty,b.pr_price as fee from codes a,prices b  where a.id=b.pr_id and a.code_type=6 and b.pr_level='standard' and a.code='".$proc_desc."'");
...
function addProcLine() {
 ...
 cell.vAlign = 'top';
 cell.innerHTML = "<b><?php echo xls('Service'); ?> " + (i + 1) + ":</b>";
 ...
 cell.innerHTML =
  ...
  " onfocus='this.blur()'" +
  " title='<?php echo xla('Click to select the desired service'); ?>'" +
...
<p>
<input type='button' value='<?php echo xla('Add Service'); ?>' onclick="addProcLine()" />
&nbsp;

...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] =="user_admin") {
    if ($_POST["mode"] == "update") {
      ...
      if ($_POST["mname"]) {
	   ...
	   if ($_POST["title"]) {
          $tqvar = formData('title','P');
          sqlStatement("update users set title='$tqvar' where id={$_POST["id"]}");
     }
	  if ($_POST["doctor_qualification"]) {
          $tqvar = formData('doctor_qualification','P');
          sqlStatement("update users set qualification='$tqvar' where id={$_POST["id"]}");
  }
   if ($_POST["doctor_category"]) {
          $tqvar = formData('doctor_category','P');
          sqlStatement("update users set category='$tqvar' where id={$_POST["id"]}");
  }
   if ($_POST["email"]) {
          $tqvar = formData('email','P');
          sqlStatement("update users set email='$tqvar' where id={$_POST["id"]}");
  }
   if ($_POST["phonecell"]) {
          $tqvar = formData('phonecell','P');
          sqlStatement("update users set phonecell='$tqvar' where id={$_POST["id"]}");
  }
...
if (isset($_POST["mode"])) {
  if ($_POST["mode"] == "new_user") {
    ...
    $insertUserSQL=            
			...
			"', phonecell = '"         . trim(formData('phonecell'        )) .
			 "', qualification = '"         . trim(formData('doctor_qualification'        )) .  
			"', category = '"         . trim(formData('doctor_category'        )) .  

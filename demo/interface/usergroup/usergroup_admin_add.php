...
<tr>
<td><span class="text"><?php xl('First Name','e'); ?>: </span></td><td><input type=entry name='fname' id='fname' maxlength='10' style="width:120px;"><span class="mandatory">&nbsp;*</span></td>
<td><span class="text"><?php xl('Middle Name','e'); ?>: </span></td><td><input type=entry name='mname' style="width:120px;"></td>
</tr>
<tr>
<td><span class="text"><?php xl('Last Name','e'); ?>: </span></td><td><input type=entry name='lname' id='lname' maxlength='10' value='LNU' style="width:120px;"><span class="mandatory">&nbsp;*</span></td>
...
<tr>
  <td>
   <span class="text"><?php xl('Doctor Qualification','e');?>: </span>
  </td>
  <td>
   <select name='doctor_qualification' id='doctor_qualification'>
<?php
$ores = sqlStatement("SELECT option_id, title FROM list_options " .
  "WHERE list_id = 'Doctor_Qualification' ORDER BY seq");
    echo "    <option value=''>-- " . xlt('SELECT') . " --\n";
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['option_id'] . "'";
  if ($orow['option_id'] == $form_title) echo " selected";
...
?>
  ...
  <td class="text"><a href='addnew_qualification.php'>Add New Qualification</a></td>
  </tr>
  <tr>
   <td>
   <span class="text"><?php xl('Doctor Category','e');?>: </span>
  </td>
  <td>
   <select name='doctor_category' id='doctor_category'>
<?php
$ores = sqlStatement("SELECT Category_id, Category_name FROM t_category_master " .
  " ORDER BY  Category_id");
    echo "    <option value=''>-- " . xlt('SELECT') . " --\n";
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['Category_id'] . "'";
  if ($orow['Category_id'] == $form_title) echo " selected";
  echo ">" . $orow['Category_name'] . "</option>\n";
}
?>
   </select>
  </td>
   <td class="text"><a href='addnew_category.php'>Add New Category</a></td>
 </tr>
 <tr>
<td><span class="text"><?php xl('Email ID','e'); ?>: </span></td><td><input type="text" name='email' id='email' onblur="ValidateEmail(this);" style="width:120px;"></td>
<td><span class="text"><?php xl('Phone Number','e'); ?>: </span></td><td><input type="text" name='phonecell' id='phonecell' maxlength='10'  style="width:120px;"></td>
</tr>
<tr>
...
<td><span class="text"><?php xl('Speciality','e'); ?>: </span></td>
<td><select style="width:400px;" name=specialty>
<?php
$fres = sqlStatement("select * from department_main order by department_name");
if ($fres) {
  for ($iter = 0;$frow = sqlFetchArray($fres);$iter++)
    $result[$iter] = $frow;
  foreach($result as $iter) {
?>
<option value="<?php echo $iter{'department_name'};?>"><?php echo $iter{'department_name'};?></option>
<?php
  }
}
?>
...
<tr>
<td><span class="text"><?php xl('Medical Council Number','e'); ?>: </span></td>
...
$('#lname').keyup(function() {
	 if(this.value.length == $(this).attr('maxlength')) {
		 $('#doctor_qualification').focus(); 
		}
	});

$('#phonecell').keyup(function() {
	 if(this.value.length == $(this).attr('maxlength')) {
		 $('#federaltaxid').focus(); 
		}
	});
...
document.getElementById('doctor_qualification').onchange = function () {
    document.getElementById('doctor_category').focus();
}
document.getElementById('doctor_category').onchange = function () {
    document.getElementById('email').focus();
}
document.getElementById('erxrole').onchange = function () {
    document.getElementById('default_warehouse').focus();
}
document.getElementById('default_warehouse').onchange = function () {
    document.getElementById('irnpool').focus();
}

...
if ($_GET["mode"] == "update") {
  ...
  if ($_GET["title"]) {
          $tqvar = formData('title','G');
          sqlStatement("update users set title='$tqvar' where id={$_GET["id"]}");
  }
   if ($_GET["doctor_qualification"]) {
          $tqvar = formData('doctor_qualification','G');
          sqlStatement("update users set qualification='$tqvar' where id={$_GET["id"]}");
  }
   if ($_GET["doctor_category"]) {
          $tqvar = formData('doctor_category','G');
          sqlStatement("update users set category='$tqvar' where id={$_GET["id"]}");
  }
   if ($_GET["email"]) {
...
while ($orow = sqlFetchArray($ores)) {
  ...
  if ($orow['option_id'] == $iter['title']) echo " selected";
...
<tr>
  ...
  <td>
   <select name='doctor_qualification'>
<?php
$ores = sqlStatement("SELECT option_id, title FROM list_options " .
  "WHERE list_id = 'Doctor_Qualification' ORDER BY seq");
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['option_id'] . "'";
  if ($orow['option_id'] == $iter['qualification']) echo " selected";
  echo ">" . $orow['title'] . "</option>\n";
}
?>
   </select>
  ...
  <td>
   <select name='doctor_category'>
<?php
$ores = sqlStatement("SELECT option_id, title FROM list_options " .
  "WHERE list_id = 'Doctor_Category_Id' ORDER BY seq");
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['option_id'] . "'";
  if ($orow['option_id'] == $iter['category']) echo " selected";
  echo ">" . $orow['title'] . "</option>\n";
}
?>
   </select>
 ...
 <tr>
<td><span class="text"><?php xl('Email ID','e'); ?>: </span></td><td><input type="text" name='email' style="width:120px;" value="<?php echo $iter["email"]; ?>"></td>
<td><span class="text"><?php xl('Phone Number','e'); ?>: </span></td><td><input type="text" name='phonecell' id='phonecell' maxlength='10' style="width:120px;" value="<?php echo $iter["phonecell"]; ?>"></td>
</tr>
...
<TR>
<TD><span class=text><?php xl('Pan Number','e'); ?>: </span></TD><TD><input type=text name=taxid style="width:150px;"  value="<?php echo $iter["federaltaxid"]?>"></td>
...
<tr>
<td><span class="text"><?php xl('MCI No','e'); ?>: </span></td><td><input type="text" name="upin" style="width:150px;" value="<?php echo $iter["upin"]?>"></td>
...
<td><select name=job style="width:400px;" >
<?php
...
foreach($result as $iter2) {
?>
  <option value="<?php echo $iter2['department_name']; ?>" <?php if ($iter['specialty'] == $iter2['department_name']) echo "selected"; ?>><?php echo htmlspecialchars($iter2['department_name']); ?></option>
<?php
}
}
?>

...
//while ($row = sqlFetchArray($result)) {
	...
	$e_Admissions->appendChild($e_Admission);

	$e_CCRDataObjectID = $ccr->createElement('CCRDataObjectID', getUuid());
	$e_Admission->appendChild($e_CCRDataObjectID);




	$e_DateTime = $ccr->createElement('DateTime');
	$e_Admission->appendChild($e_DateTime);	
	...
	$e_Admission->appendChild($e_DateTime1);
	...
	$e_Admission->appendChild($e_Diagnosis);

	$e_Text = $ccr->createElement('Text', $row['diagnosis']);
	$e_Diagnosis->appendChild($e_Text);
	
	$e_Consultant = $ccr->createElement('Consultant');
	$e_Admission->appendChild($e_Consultant);
    $prvname = $row['title']." ". $row['fname']." ". $row['lname'];
	$e_Text = $ccr->createElement('Text', $prvname);
	...
	$e_Admission->appendChild(sourceType($ccr, $sourceID));
	
	$e_Agent = $ccr->createElement('Agent');
	$e_Admission->appendChild($e_Agent);
	...
	$e_Admission->appendChild($e_Reaction);

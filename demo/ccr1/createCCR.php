...
function createCCR($action,$raw="no",$requested_by=""){
  ...
  while($res = sqlFetchArray($result[2])){
    ${"labID{$res['id']}"} = getUuid();
	   ...
	   $e_Admissions = $ccr->createElement('Admissions');
	   require_once("createCCRAdms.php");
	   $e_Body->appendChild($e_Admissions);

	   
	   $e_Clinicals = $ccr->createElement('Clinicals');
	   require_once("createCCRcClinical.php");
	   $e_Body->appendChild($e_Clinicals);

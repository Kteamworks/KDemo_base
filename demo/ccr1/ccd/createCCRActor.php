...
while ($row = sqlFetchArray($result[0])) {
	...
	$e_Age = $ccr->createElement('Age');
	$e_Person->appendChild($e_Age);
	
	$age = date_create($row['age']);
	...
	$e_ID = $ccr->createElement('ID', $row['genericname1']);
  ...
  $e_Source = $ccr->createElement('Source');
	...
	$e_Admission = $ccr->createElement('Admission');
	$e_Person->appendChild($e_Admission);
	
	$doa = date_create($row['DOB']);
	
	$e_ExactADateTime = $ccr->createElement('ExactADateTime',$doa->format('Y-m-d\TH:i:s\Z'));
	$e_Admission->appendChild($e_ExactADateTime);
...
$row1 = sqlFetchArray($result[1]);
  ...
  $e_Actor = $ccr->createElement('Actor');
	...
	$e_Name = $ccr->createElement('Name', 'Kavaii Inc');
	$e_InformationSystem->appendChild($e_Name);
  
  $e_Type = $ccr->createElement('Type', 'MedSMART');
	$e_InformationSystem->appendChild($e_Type);
  ...
  $e_Text = $ccr->createElement('Text', 'WP');
	...
	$e_Line1 = $ccr->createElement('Line1','301, Sophias Choice, St Marks Rd.');
	$e_Address->appendChild($e_Line1);
	
	$e_Line2 = $ccr->createElement('Line2');
	$e_Address->appendChild($e_Line1);

	$e_City = $ccr->createElement('City','Bangalore');
	$e_Address->appendChild($e_City);

	$e_State = $ccr->createElement('State','Kn ');
	$e_Address->appendChild($e_State);
	
	$e_PostalCode = $ccr->createElement('PostalCode','560001');
	$e_Address->appendChild($e_PostalCode);
  
  $e_Telephone = $ccr->createElement('Telephone');
	$e_Actor->appendChild($e_Telephone);
  
  $e_Phone = $ccr->createElement('Value','080-222-21879');
	$e_Telephone->appendChild($e_Phone);

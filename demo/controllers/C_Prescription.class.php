...
$pid = $_SESSION["pid"];
          $prescription = sqlStatement($qry2, array($pid));
		   			...
		   			while ( $pres=sqlFetchArray($prescription)) {
						...
						$duration .= $pres['duration'] .'  Weeks<br>';
...
//     }
	...
	function send_action($id) {
		...
		$prow = sqlQuery("SELECT pt.pharmacy_id,encounter FROM prescriptions AS rx, " .
			"patient_data AS pt WHERE rx.id = '$id' AND pt.pid = rx.patient_id");
			$encounter = $prow['encounter'];
		if ($prow['pharmacy_id']) {
    ...
    if ($GLOBALS['rx_enable_SLN']) {
        ...
        function multiprintcss_preheader() {
	        ...
	        echo (" margin-top: 40pt;\n");
                echo (" font-size: 12pt;\n");
	        echo ("}\n");
			/*echo ("div.finalsigndiv {\n");
	        echo (" margin-top: 160pt;\n");
            echo (" font-size: 12pt;\n");
	        echo ("}\n"); */
